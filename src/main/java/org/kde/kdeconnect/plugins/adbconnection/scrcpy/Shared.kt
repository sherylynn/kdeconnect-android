...
class Shared {
    ...
    enum class ListOptions(val string: String, val value: Int) {
        NULL("null", 0x0),
        ENCODERS("encoders", 0x1),
        DISPLAYS("displays", 0x2),
        CAMERAS("cameras", 0x4),
        CAMERA_SIZES("camera_sizes", 0x8),
        APPS("apps", 0x10);
        ...
        companion object {
            fun fromString(value: String) =
                entries.find { it.string.equals(value, ignoreCase = true) } ?: NULL
        }
    }
    ...
    enum class LogLevel(val string: String) {
        ...
        companion object {
            fun fromString(value: String) =
                entries.find { it.string.equals(value, ignoreCase = true) } ?: VERBOSE
        }
    }
        ...
        companion object {
            ...
            fun fromString(value: String, type: Type = Type.VIDEO) =
                entries.find { it.string.equals(value, ignoreCase = true) }
                    ?: when (type) { Type.VIDEO -> H264; Type.AUDIO -> OPUS }
    ...
    enum class VideoSource(val string: String) {
        ...
        companion object {
            fun fromString(value: String) =
                entries.find { it.string.equals(value, ignoreCase = true) } ?: DISPLAY
        }
    }
    ...
    enum class CameraFacing(val string: String) {
        ...
        companion object {
            fun fromString(value: String) =
                entries.find { it.string.equals(value, ignoreCase = true) } ?: ANY
        }
    }
    ...
    enum class Orientation(val string: String) {
        ...
        fun getRotation(): Orientation {
            return when (ordinal and 3) {
                0 -> ORIENT_0; 1 -> ORIENT_90; 2 -> ORIENT_180; 3 -> ORIENT_270
        ...
        companion object {
            fun fromInt(value: Int): Orientation =
                entries.getOrNull(value) ?: error("Invalid orientation value: $value")
        }
    }
    ...
    enum class OrientationLock(val string: String) {
        ...
        companion object {
            fun fromString(value: String) =
                entries.find { it.string.equals(value, ignoreCase = true) } ?: UNLOCKED
        }
    }
    ...
    enum class DisplayImePolicy(val string: String) {
        ...
        companion object {
            fun fromString(value: String) =
                entries.find { it.string.equals(value, ignoreCase = true) } ?: UNDEFINED
        }
    }
}
